<script lang="ts">
	...
	let messages = $state<MessageOut[]>(data.conversation.messages);
	// svelte-ignore state_referenced_locally
	...
	function attachRun(runId: string): void {
		...
		subscription = subscribeRun(runId, {
			...
			onEnded: () => {
				// Only clear active-run state if we're actually closing — some
				// browsers fire native error that routes here too.
	...
	async function handleSend(text: string): Promise<void> {
		...
		try {
			...
			conversations.bumpUpdatedAt(data.conversation.id);
